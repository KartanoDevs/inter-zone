...
- `ui/comun/` — `DialogoConfirmacion`, reutilizado para "cambios sin guardar" y para confirmar
  ...
  `gesto-tactil.ts` (spec 070): `UMBRAL_ARRASTRE_PX` y `distanciaPantalla` — antes vivían solo
  en `tablero.ts`; `ExamenTablero` los necesitó también al ganar su propia distinción
  toque/arrastre (un toque corto que no se desplaza no mueve la ficha, sin selección ni popup —
  esa parte se retiró tras probarla en real, ver "Retirada" al final de la spec 070).
