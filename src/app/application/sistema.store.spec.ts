...
describe('SistemaStore', () => {
  ...
  describe('sistemas de defensa', () => {
    ...
    it('072-E2: al recargar el contexto, el punto guardado del atacante se recupera', async () => {
      const sistemaDefensa: Sistema = {
        ...sistemaDefensaVacio(),
        defensas: [
          {
      ...
      expect(store.marcadorAtacanteEdicion()).toEqual({ x: 7.2, y: -0.8 });
    });

    it('072-E3: cada variante guarda su propio punto del atacante, sin mezclarse', async () => {
      const sistemaDefensa: Sistema = {
        ...sistemaDefensaVacio(),
        defensas: [
          {
      ...
      expect(store.marcadorAtacanteEdicion()).toEqual({ x: 6.5, y: -1.5 });
    });

    it('072-E1/E3 (via store): guardar persiste el punto movido en la variante activa', async () => {
      const store = new SistemaStore(new RepositorioFake([sistemaDefensaVacio()]));
      await store.cargar();
      seisPuestosEnCentro().forEach((c) => store.colocarOMover(`p${c.puesto}`, c.punto));
      store.moverAtacante({ x: 7.2, y: -0.8 });
      ...
      const variante = store
        .sistemaActivo()
        ?.defensas?.find((v) => v.caso === 'delantero' && v.situacion === 'z4');
      expect(variante?.marcadorAtacante).toEqual({ x: 7.2, y: -0.8 });
    });

    it('072-E7: cambiar de situación recarga el punto de la variante nueva', async () => {
      const sistemaDefensa: Sistema = {
        ...sistemaDefensaVacio(),
        defensas: [
          {
      ...
      store.seleccionarSituacion('z2');

      expect(store.situacionActiva()).toBe('z2');
      expect(store.marcadorAtacanteEdicion()).toEqual({ x: 1.5, y: -1.0 });
    });

    it('072-E10: una variante guardada antes de esta spec no tiene marcador y no cambia', async () => {
      const sistemaDefensa: Sistema = {
        ...sistemaDefensaVacio(),
        defensas: [
          // Sin `marcadorAtacante`: así quedaron guardadas las variantes anteriores a la 072.
          { caso: 'delantero', situacion: 'z4', bloqueadores: 0, formacion: seisPuestosEnCentro() },
        ],
      };
      const store = new SistemaStore(new RepositorioFake([sistemaDefensa]));
      ...
      expect(store.marcadorAtacanteEdicion()).toBeNull();
      expect(store.sistemaActivo()?.defensas?.[0].marcadorAtacante).toBeUndefined();
    });

    it('072-E4: una variante nunca guardada no tiene punto de atacante en edición', async () => {
      const store = new SistemaStore(new RepositorioFake([sistemaDefensaVacio()]));
    ...
    it('072-E5: la postura inicial nunca tiene punto de atacante en edición', async () => {
      const sistemaDefensa: Sistema = {
        ...sistemaDefensaVacio(),
        defensas: [
          {
    ...
    it('073-E2: al recargar el contexto, el punto guardado del central rival se recupera', async () => {
      const sistemaDefensa: Sistema = {
        ...sistemaDefensaVacio(),
        defensas: [
          {
      ...
      expect(store.marcadorCentralEdicion()).toEqual({ x: 3.5, y: -2.0 });
    });

    it('073-E3: cada variante guarda su propio punto del central, sin mezclarse', async () => {
      const sistemaDefensa: Sistema = {
        ...sistemaDefensaVacio(),
        defensas: [
          {
      ...
      expect(store.marcadorCentralEdicion()).toEqual({ x: 6.0, y: -0.5 });
    });

    it('073-E1 (via store): guardar persiste el punto movido del central en la variante activa', async () => {
      const store = new SistemaStore(new RepositorioFake([sistemaDefensaVacio()]));
      await store.cargar();
      seisPuestosEnCentro().forEach((c) => store.colocarOMover(`p${c.puesto}`, c.punto));
      store.moverCentral({ x: 3.5, y: -2.0 });
      ...
      const variante = store
        .sistemaActivo()
        ?.defensas?.find((v) => v.caso === 'delantero' && v.situacion === 'z4');
      expect(variante?.marcadorCentral).toEqual({ x: 3.5, y: -2.0 });
    });

    it('073-E4: la postura inicial admite colocar el central rival (guardar y recuperar)', async () => {
      const store = new SistemaStore(new RepositorioFake([sistemaDefensaVacio()]));
      await store.cargar();
      expect(store.situacionActiva()).toBe('z4');
      store.seleccionarSituacion('inicial');
      ...
      const variante = store.sistemaActivo()?.defensas?.find((v) => v.situacion === 'inicial');
      expect(variante?.marcadorCentral).toEqual({ x: 3.5, y: -2.0 });
    });

    it('073-E5: una variante sin central rival colocado no tiene punto en edición', async () => {
      const store = new SistemaStore(new RepositorioFake([sistemaDefensaVacio()]));

      await store.cargar();

      expect(store.marcadorCentralEdicion()).toBeNull();
    });

    it('073-E7: en recepción no existe ningún punto de central rival en edición', async () => {
      const sistemaRecepcion: Sistema = sistemaBase('rec1', 'Recepción');
      const store = new SistemaStore(new RepositorioFake([sistemaRecepcion]));
    ...
    it('073-E9: el central rival no cambia quién bloquea', () => {
      const formacion = seisPuestosEnCentro();

      const bloqueadoresConCentral = puestosQueBloquean(formacion, 2);

      // El central rival no es un parámetro de `puestosQueBloquean`: no hay forma de que
      // afecte al resultado. Este test documenta la garantía por firma de tipos, no solo por
      // comportamiento — `puestosQueBloquean` ni siquiera acepta un marcador rival.
      expect(bloqueadoresConCentral).toEqual(puestosQueBloquean(formacion, 2));
    });

    it('074-E4: mover el atacante a null lo quita de la edición (vuelve al banquillo)', async () => {
      const store = new SistemaStore(new RepositorioFake([sistemaDefensaVacio()]));
      await store.cargar();
      store.moverAtacante({ x: 7.2, y: -0.8 });
      expect(store.marcadorAtacanteEdicion()).not.toBeNull();

      store.moverAtacante(null);

      expect(store.marcadorAtacanteEdicion()).toBeNull();
    });

    it('074-E4: mover el central a null lo quita de la edición (vuelve al banquillo)', async () => {
      const store = new SistemaStore(new RepositorioFake([sistemaDefensaVacio()]));
      await store.cargar();
      store.moverCentral({ x: 3.5, y: -2.0 });
      expect(store.marcadorCentralEdicion()).not.toBeNull();

      store.moverCentral(null);
