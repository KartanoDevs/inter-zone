 ...
 * resultado con desglose por rotación y comparación con el modelo. Desde la spec 070 distingue
 * toque de arrastre (mismo umbral que `Tablero`, `ui/comun/gesto-tactil.ts`): un toque corto
 * que no se desplaza no mueve la ficha, así un tembleque del dedo al agarrarla no la desplaza
 * sin querer. La spec 070 añadía además una selección y un ajuste fino por pulsación larga; se
 * retiraron tras probarlos en real (ver "Retirada" al final de esa spec) — no hay panel de
 * enseñanza que enfocar en Examen, así que no queda nada más que distinguir toque de arrastre.
 */
@Component({
...
export class ExamenTablero {
   ...
   * `ui/comun/gesto-tactil.ts`): sin desplazarse más de `UMBRAL_ARRASTRE_PX`, soltar no mueve
   * la ficha — así un tembleque del dedo al agarrarla no la desplaza sin querer.
   */
