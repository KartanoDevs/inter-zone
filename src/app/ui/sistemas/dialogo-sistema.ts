...
export interface DatosSistema {
  ...
  /** Uno o más equipos (spec 048): al crear, marcar más de uno hace una copia independiente en
   * cada uno. En renombrar/clonar no se lee — el equipo no cambia una vez creado (spec 032). */
  readonly equiposId: readonly EquipoId[];
}

/**
 * Formulario de alta (crear) y edición (renombrar) de un sistema (spec 021: la defensa ya se
 * puede elegir, no solo recepción; spec 032: el equipo también; spec 048: al crear, se puede
 * marcar más de un equipo a la vez). Al renombrar o clonar, `mostrarTipo` se pone a `false` — ni
 * el tipo ni el equipo cambian una vez creado, y clonar mantiene el equipo del original.
 */
@Component({
...
export class DialogoSistema {
  ...
  /** Sin ningún equipo marcado no hay dónde crear el sistema (spec 048, E5). */
  protected readonly puedeConfirmar = computed(() => this.equiposSeleccionados().size > 0);
