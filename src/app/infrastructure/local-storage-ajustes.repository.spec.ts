...
describe('LocalStorageAjustesRepository', () => {
  it('sin nada guardado, la validación está activada por defecto', () => {
    const repositorio = new LocalStorageAjustesRepository(new AlmacenEnMemoria());

    expect(repositorio.leer()).toEqual({ validacionDesactivada: false });
  });

  it('ida y vuelta sin pérdida', () => {
    const repositorio = new LocalStorageAjustesRepository(new AlmacenEnMemoria());

    repositorio.guardar({ validacionDesactivada: true });

    expect(repositorio.leer()).toEqual({ validacionDesactivada: true });
  });
  ...
  it('una versión desconocida se ignora y devuelve el valor por defecto', () => {
    const almacen = new AlmacenEnMemoria();
    almacen.setItem('interzone.ajustes', JSON.stringify({ version: 999, data: { validacionDesactivada: true } }));
    const repositorio = new LocalStorageAjustesRepository(almacen);
