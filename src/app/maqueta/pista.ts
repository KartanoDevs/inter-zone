 ...
 * cálculo de tamaño aquí: el navegador escala solo. Incluye los 4 primeros metros del
 * campo rival (y < 0, fuera de la v1 según docs/dominio.md §3), dibujados en lila para
 * distinguirlos del campo propio; sigue sin fichas ni validación ahí.
 *
 * También hace de traductor de coordenadas: convierte puntos de pantalla (`clientX`,
 * `clientY`) a metros de pista usando `getScreenCTM()`, que ya tiene en cuenta el
 * `viewBox` y el escalado. Quien arrastra (`Tablero`) le pregunta a través de
 * `puntoDesde`/`contiene`; la pista no guarda estado de arrastre, solo hace la cuenta.
 */
@Component({
  selector: 'mqt-pista',
...
export class Pista {
   ...
   * en el DOM al agarrarlas (para traer la agarrada al frente), y capturar sobre un nodo
   * que se mueve es frágil. El `<svg>` nunca cambia de sitio.
   */
